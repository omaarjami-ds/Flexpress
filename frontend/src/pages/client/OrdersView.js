              ...
              <div key={order.id} className="order-card-glovo">
                  ...
                  <div className="order-items-list">
                    {order.items.map((item, idx) => (
                      <div key={idx} className="order-item-row">
                        ...
                        {!order.delivery_address?.startsWith('[') && item.price > 0 && <span className="item-price">{item.price.toFixed(2)} DT</span>}
                      </div>
                    ))}
                  </div>
                )}
                <div className="order-card-footer">
                  <div className="order-total">
                    ...
                    {!order.delivery_address?.startsWith('[') && order.total_price > 0 && <span className="total-amount">{order.total_price.toFixed(2)} DT</span>}
                  </div>
